 ...
 * shebang wrapper, and fuzz-smoke.sh checks the ELF magic). pglast's fuzzed code is a
 * native CPython C extension (pglast.parser, wrapping libpg_query), but the Atheris
 * libFuzzer harness driving it is a `.py`. This shim is the ELF Mayhem launches; it
...
#ifndef PY_SCRIPT
#define PY_SCRIPT "/mayhem/mayhem/fuzz_sql.py"
#endif
